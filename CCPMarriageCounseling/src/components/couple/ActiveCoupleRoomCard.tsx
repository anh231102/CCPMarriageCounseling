...
const ActiveCoupleRoomCard = ({ lengthwise }: Props) => {
  ...
  const fetchLatestRoom = async () => {
    try {
      ...
      console.error("Lỗi khi tải phòng cặp đôi:", error.message)
      setCoupleRoom(null)
    } finally {
      setLoading(false)
    }
  ...
  const handleLeaveRoom = async () => {
    try {
      ...
      fetchLatestRoom()
      setCoupleRoom((prev) =>
        prev ? { ...prev, status: CoupleRoomStatus.Cancelled } : null
      )
    } catch (error: any) {
  ...
  if (!coupleRoom) return null

  const host = coupleRoom.member
  const partner = coupleRoom.member1
  const isCancelled = coupleRoom.status === CoupleRoomStatus.Cancelled

  return (
    <View
      className={`bg-white rounded-xl p-6 mb-6 shadow-sm w-full ${
        isCancelled ? "border-l-4 border-yellow-400" : "border-l-4 border-success"
      }`}
    >
      {/* Header */}
      <View className="flex-row items-center mb-4">
        <View className={`${isCancelled ? "bg-yellow-100" : "bg-success/10"} rounded-full p-3 mr-3`}>
          <Users size={24} color={isCancelled ? "#FFC107" : "#28A745"} />
        </View>
        <View className="flex-1">
          <Text className="text-lg font-bold text-secondary-dark">Phòng đang tham gia</Text>
          <Text
            className={`text-sm font-medium ${
              isCancelled ? "text-yellow-600" : "text-success"
            }`}
          >
            {getCoupleRoomStatusLabel(coupleRoom.status as CoupleRoomStatus)}
          </Text>
          <View
            className={`rounded-lg px-3 py-1 w-36 mt-2 ${
              isCancelled ? "bg-yellow-100" : "bg-success/10"
            }`}
          >
            <Text
              className={`font-bold  ${
      ...
      {/* Nếu phòng đã bị huỷ, hiển thị thông báo */}
      {isCancelled ? (
        <View className="bg-yellow-50 rounded-xl p-4">
          <View className="flex-row items-center mb-2">
            <Crown size={16} color="#FFC107" />
            <Text className="text-yellow-600 font-medium ml-2">
              Bạn hiện không có phòng hoạt động
            </Text>
          </View>
