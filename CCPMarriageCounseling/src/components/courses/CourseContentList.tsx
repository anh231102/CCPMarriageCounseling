...
import { View, Text, TouchableOpacity, Alert, type ColorValue } from "react-native"
import {
  Play,
  BookOpen,
  HelpCircle,
...
export const CourseContentList: React.FC<CourseContentListProps> = ({ chapters, isEnrolled, onPressChapter }) => {
  ...
  const previewChapters = !isEnrolled ? sortedChapters.filter((ch) => ch.status === 1).slice(0, 2) : []

  // Calculate progress
  const completedChapters = sortedChapters.filter((chapter) => chapter.isDone).length
  ...
  return (
    <View className="bg-white rounded-lg p-4 shadow-sm mb-4 mx-4">
      ...
      {isEnrolled && totalChapters > 0 && (
        <View className="mb-4 bg-gradient-to-r from-pink-50 to-purple-50 rounded-2xl p-4">
          <View className="flex-row items-center justify-between mb-3">
            <View className="flex-row items-center">
              <View className="bg-pink-100 p-2 rounded-xl mr-3">
                <TrendingUp size={18} color="#E83E8C" />
              </View>
              <View>
                <Text className="text-gray-800 font-bold text-base">Tiến độ học tập</Text>
                <Text className="text-gray-600 text-sm">
                  {completedChapters}/{totalChapters} chương đã hoàn thành
                </Text>
              </View>
            </View>
            <View className="bg-white px-3 py-2 rounded-xl border border-pink-200">
              <Text className="text-pink-600 font-bold text-lg">{Math.round(progressPercentage)}%</Text>
            </View>
          </View>
          ...
          <View className="flex-row justify-between mt-2">
            <Text className="text-gray-500 text-xs">Bắt đầu</Text>
                  ...
                  >
                    <View className="flex-row items-center">
                      ...
                      <View className="flex-1 mr-3">
                        ...
                        {chapter.description && (
                          <Text className={`text-sm ${canPress ? "text-gray-600" : "text-gray-400"}`} numberOfLines={2}>
                            {chapter.description}
                          </Text>
                        )}
                      ...
                      >
                        {statusInfo.icon}
                        <Text className={`${statusInfo.textColor} text-xs font-semibold ml-1`}>{statusInfo.text}</Text>
                      </View>
                    </View>
                  </View>
                </TouchableOpacity>
