      ...
      <ScrollView className="flex-1 px-4 -mt-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white rounded-xl shadow-sm p-4 mb-6">
          ...
          >
            ...
            {showAllDetails ? <ChevronUp size={18} color="#E83E8C" /> : <ChevronDown size={18} color="#E83E8C" />}
          </TouchableOpacity>

          {showAllDetails && (
            <View className="mt-3">
              {surveyTypes.map(type => (
                <View key={type.id} className="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-200">
                  <Text className="text-primary font-semibold text-base">{type.name} ({type.fullNameVi})</Text>
        ...
        <View className="bg-white rounded-xl shadow-sm p-6 mb-6">
          ...
          <View className="space-y-3">
            {["Bản đồ tình yêu chi tiết", "Điểm tương thích tổng hợp", "Lời khuyên cá nhân hóa"].map((title, idx) => (
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-secondary text-sm">
                    {idx === 0
                      ...
                      : "Gợi ý cụ thể để cải thiện và phát triển mối quan hệ"}
                  </Text>
                </View>
              </View>
            ))}
